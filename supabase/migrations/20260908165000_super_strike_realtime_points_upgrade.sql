...
-- Applied to HYPHSWORLD production Supabase project yuhxtdkhsltaqiagrtys on 2026-09-08.
...
declare
  ...
  v_balance integer := 0;
begin
  if p_user_id is null or p_amount <= 0 or p_amount > 20 then raise exception 'invalid_super_strike_reward'; end if;
  ...
  select coalesce(sum(greatest(l.amount, 0)), 0)::integer, max(l.created_at)
  into v_today, v_last
  ...
  if p_source = 'super_strike_complete' and v_last is not null and v_last > now() - interval '90 seconds' then
    select coalesce(p.cool_points, p.points, 0) into v_balance
    from public.profiles p where p.id = p_user_id;
  ...
  insert into public.cool_points_ledger (
    ...
    p_user_id, p_amount, p_reason, p_source,
    jsonb_build_object('context', coalesce(p_context, ''), 'game_key', 'super_strike', 'version', 2), now()
  );
...
begin
  ...
  v_state := jsonb_set(
    ...
    jsonb_build_object('current_frame',p_current_frame,'finished',coalesce(p_finished,false),'updated_at',now()),
    true
  );
  ...
  if (select count(*) from public.game_players where room_id=v_room.id and status='ready')=2 then
    ...
    if v_finished_now is not null then
      select max(score), min(score) into v_max_score, v_min_score
      from public.game_players where room_id=v_room.id and status<>'left';
      ...
      loop
        v_reward := 5;
        if v_max_score <> v_min_score and v_player.score = v_max_score then v_reward := 8; end if;
        perform private.award_super_strike_points(
          v_player.user_id, v_reward, 'super_strike_multiplayer', v_room.id::text,
...
begin
  ...
  where gs.user_id=v_user_id and gs.game_key=v_game_key
  ...
  insert into public.game_scores (
    ...
    v_user_id, v_game_key, v_score, v_points,
    coalesce(p_metadata,'{}'::jsonb) || jsonb_build_object('points_awarded',v_points), now()
  );
  ...
  return jsonb_build_object(
    'ok',true,'game_key',v_game_key,'score',v_score,'points_delta',v_points,'balance',v_balance,
