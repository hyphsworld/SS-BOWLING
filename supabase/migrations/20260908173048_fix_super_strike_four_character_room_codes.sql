...
begin
  ...
  loop
    ...
    begin
      ...
      values('super_strike',v_user,2,v_code)
  ...
  values(v_room.id,v_user,1,'joined');

  insert into public.game_state(room_id,state,updated_by)
  values(v_room.id,'{"progress":{}}'::jsonb,v_user);
...
begin
  ...
  where game_type='super_strike'
    ...
    and (
      ...
      or (
        ...
        and room_code like v_code || '%'
        and 1 = (
          select count(*) from public.game_rooms r2
          where r2.game_type='super_strike'
            and r2.status in ('waiting','playing')
            and r2.room_code like v_code || '%'
        )
      )
    )
  ...
  if v_count>=2 and not exists(
    select 1 from public.game_players where room_id=v_room.id and user_id=v_user
  ) then raise exception 'room_full'; end if;

  insert into public.profiles(id) values(v_user) on conflict(id) do nothing;

  insert into public.game_players(room_id,user_id,seat_number,status)
  values(v_room.id,v_user,2,'joined')
